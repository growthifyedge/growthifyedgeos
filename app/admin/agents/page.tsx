...
import { EditAgentDialog } from "@/components/app/edit-agent-dialog";
import { DeleteAgentButton } from "@/components/app/delete-agent-button";
import {
  Card,
  CardContent,
  CardHeader,
...
export default async function AgentsPage() {
  ...
  const [me, peopleRes, settingsRes, agencyRes, adminCountRes] =
    await Promise.all([
      getCurrentProfile(),
      supabase
        .from("profiles")
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage agents, shifts, targets and payroll.
          </p>
        </div>
        <CreateAgentDialog />
      </div>
                  ...
                  return (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-sm">
                        {p.shift_start_time?.slice(0, 5)} –{" "}
                        {p.shift_end_time?.slice(0, 5)}
                      </TableCell>
                      <TableCell className="text-sm">
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end">
                          <EditAgentDialog profile={p} settings={s} />
                          <DeleteAgentButton
                            id={p.id}
                            name={p.full_name || p.email || "this agent"}
